...
const ytcfgHomepage = JSON.parse(
	readFileSync(
		fileURLToPath(new URL('../../dummy/ytcfgHomepage.json', import.meta.url)),
		'utf8',
	),
) as { html: string };
...
const multiCallOneLine
	= '<script>ytcfg.set(0); ytcfg.set({"INNERTUBE_API_KEY":"abc","note":"has ); inside"}); ytcfg.set({"OTHER":"second );"});</script>';
...
test('extractYtcfgConfig (YMOO-1): extracts exactly the first config object from one-line multi-call HTML', (t) => {
	const config = extractYtcfgConfig(multiCallOneLine) as unknown as Record<string, unknown>;
...
test('extractYtcfgConfig (YMOO-1): parses real captured YouTube homepage HTML', (t) => {
	const config = extractYtcfgConfig(ytcfgHomepage.html) as unknown as Record<string, unknown>;
...
test('extractYtcfgConfig (YMOO-2): throws a typed IllegalStateError when no config is present', (t) => {
	t.throws(
		() => extractYtcfgConfig(noConfig),
		IllegalStateError,
		'throws IllegalStateError rather than a raw SyntaxError',
	);
	t.end();
});
